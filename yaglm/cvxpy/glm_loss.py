...
def lin_reg_loss(X, y, coef, intercept=None, offsets=None):
    """
...
def l2_reg_loss(X, y, coef, intercept=None, offsets=None):
...
def log_reg_loss(X, y, coef, intercept=None, offsets=None):
    z = get_z(X, coef, intercept=intercept, offsets=offsets)

    return (1 / X.shape[0]) * cp.sum(cp.logistic(z) - cp.multiply(y, z))


def multinomial_loss(X, y, coef, intercept=None, offsets=None):
...
def poisson_reg_loss(X, y, coef, intercept=None, offsets=None):
    z = get_z(X, coef, intercept=intercept, offsets=offsets)

    return (1 / X.shape[0]) * cp.sum(cp.exp(z) - cp.multiply(y, z))


def quantile_reg_loss(X, y, coef, intercept=None, offsets=None, quantile=0.5):
    z = get_z(X, coef, intercept=intercept, offsets=offsets)
...
def huber_reg_loss(X, y, coef, intercept=None, offsets=None, knot=1):
    z = get_z(X, coef, intercept=intercept, offsets=offsets)
