...
from ya_glm.solver.quantile_lp.utils import get_lin_prog_data, \
    get_coef_inter, get_quad_mat


def solve(X, y, fit_intercept=True, quantile=0.5, sample_weight=None,
          lasso_pen_val=1, ridge_pen_val=None,
    ...
    problem, var, lasso_pen_val, ridge_pen_val = \
...
def solve_path(fit_intercept=True, cp_kws={}, zero_tol=1e-8,
    ...
    start_time = time()
    problem, var, lasso_pen_val, ridge_pen_val = setup_problem(**kws)
...
def setup_problem(X, y, fit_intercept=True, quantile=0.5, sample_weight=None,
    ...
    if lasso_pen_val is not None:
        lasso_pen_val = cp.Parameter(pos=True, value=lasso_pen_val)

    if ridge_pen_val is not None:
        ridge_pen_val = cp.Parameter(pos=True, value=ridge_pen_val)
    ...
    return problem, var, lasso_pen_val, ridge_pen_val
