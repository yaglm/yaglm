...
class Ridge(LossMixin, GlmCvxPen):
    """
    A GLM with a ridge penalty e.g. either the squared L2 norm or the L2 norm with a Tikhonov matrix.
    ...
    def __init__(self, loss='lin_reg', fit_intercept=True,
                 pen_val=1, ridge_weights=None, tikhonov=None,
    ...
    def _get_penalty_config(self):
        ...
        """
        return ConvexPenalty(ridge_pen_val=self.pen_val,
...
class RidgeCV(SinglePenSeqSetterMixin, RunCVGridOrPathMixin, GlmCV):
    """
    Tunes a ridge penalized GLM using cross-validation. Makes use of a path algorithm if the solver object has one available.

    Parameters
    ----------
    estimator: ya_glm.models.Ridge
        The base Ridge estimator to be tuned with cross-validation. Only the pen_val parameter is tuned.
