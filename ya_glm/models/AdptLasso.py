...
class AdptLasso(LossMixin, GlmAdptPen):
    """
    A GLM with a Adaptive Lasso-like penalty e.g. Lasso, group Lasso, multi-task Lasso, or nuclear norm.

    An adpative penalty is a two stage estimator. In the first stage an initial estimate of the coefficient is fit (e.g. using LassoCV). In the second stage, a weighted Lasso estimator is fit where the 'adaptive weights' are obtained from the initial coefficient.
...
class AdptLassoCV(SinglePenSeqSetterMixin, RunCVGridOrPathMixin, GlmAdptPenCV):
    """
    Tunes an AdpativeLasso-like penalized GLM using cross-validation. Makes use of a path algorithm if the solver object has one available.

    Note the initializer is fit before running cross-validation so the same adpat_weights are used for each CV fold.

    Parameters
    ----------
    estimator: ya_glm.models.AdptLasso
        The base Adpative Lasso estimator to be tuned with cross-validation. Only the pen_val parameter is tuned.
